...
type configPoller struct {
	...
	// memcache client used to query the config endpoint
	mc *Client
...
// newConfigPoller creates a new cluster config poller.
...
// stopPolling stops the internal polling.
...
func (c *configPoller) readConfigAndUpdateServerList() error {
	...
	if updateClusterConf {
		c.clusterConfigMU.Lock()
		err := c.updateServerList(clusterConfig)
		c.clusterConfigMU.Unlock()
